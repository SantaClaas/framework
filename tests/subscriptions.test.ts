...
type ExpectedDifferentiationResult = {
...
function toIdsWithoutDuplicates({
  duplicates,
  toStop,
  toKeep,
  toStart,
...
function toIds({
  ...
  toKeep,
  toStart,
}: ExpectedDifferentiationResult): ResultToCompare {
  return {
...
function equals(
  expected: ExpectedDifferentiationResult,
  actual: DifferentiationResult<void>,
) {
  const idsExpected = toIds(expected);
  const idsActual = toIdsWithoutDuplicates(actual);
...
describe('Differentiation behavior', () => {
  ...
  test('No changes when subscriptions and active subscriptions are the same', () => {
    ...
    const expected: ExpectedDifferentiationResult = {
      duplicates: [],
      toStop: [],
